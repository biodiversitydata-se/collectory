...
class CollectoryTagLib {
    ...
    def ifAllGranted = { attrs, body ->
        def granted = true
        if (!grailsApplication.config.security.cas.enabled.toBoolean()) {
            granted = true
        } else {
    ...
    def ifGranted = { attrs, body ->
        if (!grailsApplication.config.security.cas.enabled.toBoolean() || request.isUserInRole(attrs.role as String)) {
            out << body()
        }
    }
    ...
    def isNotLoggedIn = {attrs, body ->
        if (grailsApplication.config.security.cas.enabled.toBoolean() && !request.getUserPrincipal()) {
            out << body()
        }
    }
    ...
    private boolean isAdmin() {
        return !grailsApplication.config.security.cas.enabled.toBoolean() || request?.isUserInRole(grailsApplication.config.ROLE_ADMIN as String)
    }
